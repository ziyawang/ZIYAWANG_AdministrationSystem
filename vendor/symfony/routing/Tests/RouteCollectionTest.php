...
class RouteCollectionTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testIterator()
    {
        ...
        $collection1->add('foo', $foo = new Route('/foo-news'));
        $collection->addCollection($collection1);
    ...
    public function testAddDefaultsAndRequirementsAndOptions()
    {
        ...
        $collection->addDefaults(array('placeholder' => 'news-default'));
        $this->assertEquals(array('placeholder' => 'news-default'), $collection->get('foo')->getDefaults(), '->addDefaults() adds defaults to all routes');
        $this->assertEquals(array('_controller' => 'fixed', 'placeholder' => 'news-default'), $collection->get('bar')->getDefaults(),
        ...
        $collection->addOptions(array('option' => 'news-value'));
        $this->assertEquals(
            array('option' => 'news-value', 'compiler_class' => 'Symfony\\Component\\Routing\\RouteCompiler'),
    ...
    public function testUniqueRouteWithGivenName()
    {
        ...
        $collection3->add('foo', $new = new Route('/news'));

        $collection2->addCollection($collection3);
        $collection1->addCollection($collection2);

        $this->assertSame($new, $collection1->get('foo'), '->get() returns news route that overrode previous one');
        // size of 1 because collection1 contains /news but not /old anymore
        $this->assertCount(1, $collection1->getIterator(), '->addCollection() removes previous routes when adding news routes with the same name');
    }
